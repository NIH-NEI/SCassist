# Shared helpers for SCassist InteractionAgent analyses.
...
SCassist_interactions_validate_static_inputs <- function(species,
  ...
  if (!is.character(database_scope) || length(database_scope) != 1 ||
    ...
    stop("Error: unsupported database_scope. InteractionAgent currently supports only 'default'.", call. = FALSE)
  }
